	...
	namespace Gfx
	{
		namespace ParticleUpdaters
		{
			void EulerUpdater::Update(float delta, ParticleArray* particles)
			{
				...
				for (uint i = 0; i < end; i++)
				{
					particles->Velocities[i] += Maths::Vector4f(delta * particles->Accelerations[i].x, delta * particles->Accelerations[i].y, delta * particles->Accelerations[i].z, delta * particles->Accelerations[i].w);
				}
